...
RSpec.describe "LegacyAPI::Domains#index", type: :request do
  ...
  let!(:server_domain) { create(:domain, owner: server, name: "server-domain.example") }
  let!(:organization_domain) { create(:domain, owner: organization, name: "organization-domain.example") }
  ...
  it "returns domains across organizations for admin credentials" do
  ...
  it "includes domains from assigned organizations for non-admin owners" do
    ...
    expect(json["status"]).to eq("success")
    expect(names).to include(other_org_domain.name)
  end
